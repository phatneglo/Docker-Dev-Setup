# ONLYOFFICE Document Server - Windows Docker Setup (Port 8080)
...
Write-Host "Starting ONLYOFFICE Document Server on port 8080..." -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Green
...
# Run ONLYOFFICE Document Server with data persistence on port 8080
Write-Host "Starting ONLYOFFICE Document Server on port 8080..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "============================================" -ForegroundColor Green
    Write-Host "Document Server: http://localhost:8080" -ForegroundColor Cyan
    Write-Host "API Endpoint: http://localhost:8080/web-apps/apps/api/documents/api.js" -ForegroundColor Cyan
    Write-Host "Health Check: http://localhost:8080/healthcheck" -ForegroundColor Cyan
    ...
    # Test health check
    try {
        $response = Invoke-WebRequest -Uri "http://localhost:8080/healthcheck" -TimeoutSec 30
        if ($response.StatusCode -eq 200) {
            Write-Host "Health check passed! Service is ready." -ForegroundColor Green
            Write-Host "You can now open test.html to test the document editor." -ForegroundColor Green
        }
    }
    catch {
        Write-Host "Service is starting up. Health check will be available shortly." -ForegroundColor Yellow
        Write-Host "Try accessing http://localhost:8080/healthcheck in a few minutes." -ForegroundColor Yellow
    }
